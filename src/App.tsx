...
function AiFeatures() {
  ...
  return <section className="section features"><div className="wrap"><div className="section-head reveal"><div><p className="section-index">04 / AI助手功能</p><h2>把复杂判断，转化为清晰下一步</h2></div></div><div className="feature-grid reveal">{items.map(([no, title, text]) => <article key={no}><span>{no}</span><div className={`feature-icon icon-${no}`} aria-hidden="true"><i /><i /><i /></div><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
}
...
function UsageGuide() { return <section className="section usage"><div className="wrap"><div className="section-head reveal"><div><p className="section-index">06 / 使用流程</p><h2>如何使用AI解雇SOP助手</h2></div><p>三步进入风险识别</p></div><div className="usage-grid reveal">{siteConfig.usageImages.map((src, i) => <article key={src}><div className="step-top"><b>步骤 {String(i + 1).padStart(2, '0')}</b><span>0{i + 1}</span></div><SafeImage src={src} label={siteConfig.usagePlaceholders[i]} /></article>)}</div></div></section> }

function QrCodeSection() { return <section className="qr-section"><div className="wrap qr-grid reveal"><div><p className="section-index light">07 / 获取助手</p><h2>让每一次解除决策，<br />都有据可循。</h2><p>扫码关注后，即可进入AI解雇SOP助手</p></div><div className="qr-card"><SafeImage src={siteConfig.qrCode} label="公众号二维码待补充" /><strong>公众号二维码待补充</strong></div></div><p className="disclaimer wrap">AI助手提供的内容仅用于一般性法律信息及初步风险识别，不构成针对具体事项的正式法律意见。涉及具体员工处理时，请结合完整事实、证据及当地司法实践，由专业律师进一步复核。</p></section> }
...
export default function App() {
  ...
  return <><Hero /><main><WhitepaperOverview /><ChapterMap /><AiTransition /><AiFeatures /><QuestionBank /><UsageGuide /><QrCodeSection /></main><Footer /></>
}
